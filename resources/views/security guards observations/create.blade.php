@extends('layouts.app')
@section('title')
    {{ __('New Security Guard') }}
@endsection
@section('page-title')
    {{ __('Add New Security Guard') }}
@endsection
...
@section('content')
    ...
    <div class="row m-2">
        <div class="col-lg-12 margin-tb">
            <div class="pull-right">
                <a class="btn btn-primary" href="{{ route('security.guards.index') }}">{{ __('Back') }}</a>
            </div>
        </div>
    </div>
    ...
    <div class="container text-capitalize text-center" style="font-family: sans-serif">
        ...
        <form class="row m-auto text-center" action="{{ route('worker.store') }}" method="POST"
                ...
                <div class="col-md-3 m-auto">
                    <div class="form-floating">
                        <select name="title_id" id="title" class="form-select text-capitalize">
                            <option value="" class="text-capitalize" selected hidden>{{ __('choose worker title') }}
                            </option>
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        data.forEach(function(row) {
                            $('#location').find('option')
                                .remove();
                            selectLocation += '<Option value=' + row.id + '>' + row
