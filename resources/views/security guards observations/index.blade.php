...
@section('content')
    ...
    @can('Guards Observations Create')
        <div class="row">
            <div class="col-lg-12 m-2">
                <div class="pull-right">
                    <a class="btn btn-success" href="{{ route('guards.observations.create') }}">
                        {{ __('Create New Observation') }}</a>
                </div>
            </div>
        </div>
    @endcan
                    ...
                    @foreach ($workers as $worker)
                        <tr>
                            ...
                            <td class="text-capitalize">
                                @if (isset($worker->img))
                                    <img src='{{ asset("media/workers/$worker->img") }}' class="" width="50"
                                        height="50">
                                @else
                                    <img src='{{ asset('assets/images/temp.png') }}' class="" width="50"
                                        height="50">
                                @endif
                            </td>
                                    ...
                                    <ul class="dropdown-menu text-capitalize" aria-labelledby="dropdownMenuButton1">
                                        @can('Worker Show')
                                            <li><a class="dropdown-item"
                                                    href="{{ route('worker.show', $worker->id) }}">Show</a></li>
                                        @endcan
                                        @can('Worker Edit')
                                            <li><a class="dropdown-item"
                                                    href="{{ route('worker.edit', $worker->id) }}">Edit</a></li>
                                        @endcan
                                        @can('Worker Delete')
                                            <li><a class="dropdown-item"
                                                    href="{{ route('worker.destroy', $worker->id) }}">Remove</a>
                                            </li>
                                        @endcan
                                    </ul>
                                </div>
