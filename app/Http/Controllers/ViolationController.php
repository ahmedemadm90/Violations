...
/*         $this->middleware('permission:Violations|Violations List|Violation Create|Violation Edit|User Delete', ['only' => ['index', 'store']]);
    ...
    {
        $violations = Violation::where('nearmiss', $request->keyword)
            ->orwhere('involved_ids', 'like', "%$request->keyword%")
            ->orwhere('involved_names', 'like', "%$request->keyword%")
            ->paginate(50);
        return view('violations.index', [
            "violations" => $violations,
        ]);
    ...
    {
        $egy = Country::where('country_name', 'Egy')->first();
        $vps = Vp::where('region_id', 1)->orderby('vp_name')->get();
        $areas = Area::where('vp_id', $request->id)->orderBy('area_name');
        $workers = Worker::select('id', 'name', 'company_id')->where('country_id', $egy->id)->get();
    ...
    public function store(Request $request)
    {
        ...
        $input['type'] = $request->classification;
